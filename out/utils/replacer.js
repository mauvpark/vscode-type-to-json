 ...
 */
exports.default = (text, defaultValues) => {
    ...
    const stringReplaced = onlyKeyTypeText.replace(regexString, defaultValues?.string ? `${defaultValues.string}` : `\"\"`);
    // number
    const regexNumber = /number/g;
    const numberReplaced = stringReplaced.replace(regexNumber, defaultValues?.number ? `${defaultValues.number}` : "0");
    // boolean
    const regexBoolean = /(boolean)|(Boolean)/g;
    const booleanReplaced = numberReplaced.replace(regexBoolean, defaultValues?.boolean ? `${defaultValues.boolean}` : "false");
    // array
    const regexArray = /(\[\]) | (Array)/g;
    const arrayReplaced = booleanReplaced.replace(regexArray, defaultValues?.array ? `${defaultValues.array}` : "[]");
    // object
    ...
    const undefinedReplaced = nullReplaced.replace(regexUndefined, `\"undefined\"`);
    // comment
    ...
    const noObjAndSemicolonPatternList = spaceReplaced.match(regexNoObjAndSemicolon);
    let noObjAndSemicolonReplaced = spaceReplaced;
    noObjAndSemicolonPatternList?.forEach((pattern) => (noObjAndSemicolonReplaced = noObjAndSemicolonReplaced.replace(pattern, pattern.replace(";", ","))));
    ...
    objAndSemicolonPatternList?.forEach((pattern) => (objAndSemicolonReplaced = objAndSemicolonReplaced.replace(pattern, pattern.replace(";", ","))));
